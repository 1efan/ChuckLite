...
public final class ChuckLiteConfig {
    ...
    private static void setDefaults(Properties p) {
        p.putIfAbsent("throttle.enabled", "false");
        p.putIfAbsent("throttle.maxPerTick", "12");

        // Adaptive frame-budget throttling: on 26.1 the chunk-batch rate (chunks per tick) is scaled
        // to whatever the current frame rate can afford instead of a fixed cap. Bounds are in
        // chunks-per-tick, sized for 26.1's batch lever (not the GPU-upload lever used on 1.20.1/1.21.1).
        p.putIfAbsent("throttle.adaptive", "true");
        p.putIfAbsent("throttle.targetFps", "60");
        p.putIfAbsent("throttle.minPerFrame", "2");
        p.putIfAbsent("throttle.maxPerFrame", "25");

        p.putIfAbsent("directional.enabled", "false");
        p.putIfAbsent("directional.retentionAngle", "120");

        p.putIfAbsent("memory.enabled", "false");
    ...
    public static boolean adaptiveThrottle()      { return getBool("throttle.adaptive", true); }
    public static int throttleTargetFps()         { return getInt("throttle.targetFps", 60, 15, 240); }
    public static int throttleMinPerFrame()       { return getInt("throttle.minPerFrame", 2, 1, 50); }
    public static int throttleMaxPerFrame()       { return getInt("throttle.maxPerFrame", 25, 1, 50); }
