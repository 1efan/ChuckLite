...
public final class ChuckLiteConfig {
    ...
    private static void setDefaults(Properties p) {
        ...
        // Adaptive frame-budget throttling: scale chunk work to whatever the current frame rate can
        // afford instead of a fixed per-tick cap.
        p.putIfAbsent("throttle.adaptive", "true");
        p.putIfAbsent("throttle.targetFps", "60");
        p.putIfAbsent("throttle.minPerFrame", "4");
        p.putIfAbsent("throttle.maxPerFrame", "64");
    ...
    public static int throttleMinPerFrame()       { return getInt("throttle.minPerFrame", 4, 1, 64); }
    public static int throttleMaxPerFrame()       { return getInt("throttle.maxPerFrame", 64, 1, 256); }
